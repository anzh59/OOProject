...
namespace OOProject
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Vector aVector = new Vector(6) { Printer = new ConsolePrinter() };

            for (int i = 0; i < aVector.ElementsCount; i++)
            {
                aVector[i] = random.Next(30);
            }
            ...
            Vector bVector = new Vector(6) { Printer = new ConsolePrinter() };

            for (int i = 0; i < bVector.ElementsCount; i++)
            {
                bVector[i] = random.Next(30);
            }
